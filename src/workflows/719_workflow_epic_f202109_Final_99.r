...
HT_tuning_epic <- function( pinputexps, bypass=FALSE)
{
  ...
  param_local$bo_iteraciones <- 30 #ieraciones de la Optimizacion Bayesiana
...
wf_septiembre <- function( pnombrewf )
{
  ...
  fm <- FM_final_models_lightgbm_semillerio( c(ht, ts9), ranks=c(1), semillerio=20,repeticiones_exp=15 )
